...
**Live Application:** [https://footyliveliness.com](https://footyliveliness.com)
...
│   │   └── SCRAPER_SUMMARY.md         # Scraper summary
│   │
...
│   │       ├── scrape_upcoming_fixtures.py    # Upcoming matches scraper
│   │       ├── src/                   # React frontend
...
│   │       ├── past_season_rankings.json  # Historical rankings
...
- **Top Right (Test MAE):** Simple xG also has the lowest Mean Absolute Error, making predictions closest to actual values.
- **Middle Left (Overfitting Check):** All metrics cluster near the perfect fit line, indicating good generalization without overfitting.
- **Middle Right (Ranking Quality):** Simple xG has the highest Spearman correlation (0.896), meaning it ranks matches in the correct order 90% of the time.
- **Bottom Right (Top-10 Hit Rate):** Simple xG correctly identifies 90% of the most exciting matches, crucial for the app's primary use case.

**Why Simple xG won:** It combines total action (xG_total) with competitive balance (minimum xG between teams), capturing both high-scoring games and close contests.
...
- **Top Right (Test MAE):** ElasticNet has the lowest prediction error (0.452), making it the most accurate model. Lower is better.
- **Bottom Left (Overfitting Check):** ElasticNet (orange) sits closest to the perfect fit line, showing excellent generalization. GradientBoosting (green) shows slight overfitting as it's further from the line.
- **Bottom Right (Top-10 Hit Rate):** All three models achieve 90% accuracy in identifying the most exciting matches, but ElasticNet does it with better overall performance.

**Why ElasticNet won:** It combines L1 (Lasso) and L2 (Ridge) regularization, preventing overfitting while maintaining high accuracy. It's also interpretable and computationally efficient.
...
- **Right (Top-10 Hit Rate):** Single-season model achieves 90% accuracy vs ~20% for multi-season models, proving that recent data is far more predictive than historical data.

**Key insight:** Training only on the current season (2024/25) produces a model that's 8x more accurate than using multiple seasons. This validates our decision to focus on single-season data.
